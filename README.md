...
When a scan is submitted with `aiGeneratedText: { detect: true }` and AI-generated text is found, the completed webhook contains a `suspected-ai-text` alert in `notifications.alerts`. The alert's `additionalData` is a JSON string. The SDK decodes it into a typed `CopyleaksAiTextDetectionResponseModel`.

```javascript
const express = require('express');
const { CompletedWebhookModel } = require('plagiarism-checker');

const app = express();
// The completed webhook can be large, so raise the default body size limit.
...
```
A missing AI alert means the scan produced no AI alert. It does not by itself prove that AI detection ran: check the other AI detection alert codes in `CopyleaksAlertCodes` (for example `AI_DETECTION_FAILED` or `AI_DETECTION_TEXT_TOO_SHORT`). The raw JSON string stays available in `aiAlert.additionalData`.
##
